    ...
    <div className="bg-white rounded-xl border border-amber-200 overflow-hidden">
      ...
      <div className="px-5 py-4 bg-amber-50 border-b border-amber-200 flex items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-amber-800">
            ...
            <span className="ml-2 font-normal text-amber-600">({uncategorized.length} transaction{uncategorized.length !== 1 ? 's' : ''})</span>
          </h3>
        </div>
        <div className="flex items-center gap-2">
          <button
      ...
      <div className="divide-y divide-gray-50">
        {Object.entries(grouped).map(([description, expenses]) => (
          <div key={description} className="px-5 py-3 flex items-center gap-4">
            {/* Description + meta */}
            ...
            <label className="flex items-center gap-1.5 text-xs text-gray-500 cursor-pointer whitespace-nowrap flex-shrink-0">
              <input
                type="checkbox"
                checked={saveAsRule[description] !== false}
                onChange={(e) =>
                  setSaveAsRule((s) => ({ ...s, [description]: e.target.checked }))
                }
            ...
            <select
              ...
              className="w-44 border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white flex-shrink-0"
            >
            ...
            <button
              ...
              className="px-3 py-1.5 bg-blue-600 text-white text-xs rounded-lg hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors font-medium flex-shrink-0"
            >
              Apply
            </button>
          </div>
